 ...
 * rotation possible without a restart. The most recent public keys are retained
 * and selected by the token's {@code kid}, so tokens issued just before a rotation
 * keep verifying until they expire without unbounded memory growth.</p>
 *
...
public class AwsSecretsManagerJwtKeyProvider implements JwtKeyProvider {
    ...
    private static final int MAX_KNOWN_PUBLIC_KEYS = 8;
    ...
    /** Recent public keys, so a rotation does not break live tokens indefinitely. */
    private final Map<String, RSAPublicKey> knownPublicKeys =
    ...
    private void load() {
        ...
        while (knownPublicKeys.size() > MAX_KNOWN_PUBLIC_KEYS) {
            knownPublicKeys.remove(knownPublicKeys.keySet().iterator().next());
        }
